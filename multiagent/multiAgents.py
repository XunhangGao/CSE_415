...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        for action in gameState.getLegalActions(0):
            ...
            actions[action] = self.minValue(successor, 0, 1)
    ...
    def minValue(self, state, depth, index):
        ...
        for action in state.getLegalActions(index):
            ...
            if index == state.getNumAgents() - 1:
                ...
                v = min(v, self.minValue(successor, depth, index + 1))
        return v
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        for action in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, action)
            actions[action] = self.minValue(successor, 0, 1, alpha, beta)
            alpha = max(alpha, actions[action])
        return actions.argMax()
        """
        return self.maxValue(gameState, 0, alpha, beta)
        """
    ...
    def minValue(self, state, depth, index, alpha, beta):
        ...
        for action in state.getLegalActions(index):
            ...
            if index == state.getNumAgents() - 1:
                ...
                v = min(v, self.minValue(successor, depth, index + 1, alpha, beta))
            if v < alpha:
                return v
            beta = min(beta, v)
        return v
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        for action in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, action)
            actions[action] = self.expValue(successor, 0, 1)
        return actions.argMax()
        """
        return self.maxValue(gameState, 0)
        """
    ...
    def maxValue(self, state, depth):
        ...
        for action in state.getLegalActions(0):
            successor = state.generateSuccessor(0, action)
            temp = self.expValue(successor, depth, 1)
            actions[action] = temp
            v = max(v, temp)
        if depth == 0:
            return actions.argMax()
    ...
    def expValue(self, state, depth, index):
        ...
        for action in state.getLegalActions(index):
            successor = state.generateSuccessor(index, action)
            if index == state.getNumAgents() - 1:
                temp = self.maxValue(successor, depth + 1)
            else:
                temp = self.expValue(successor, depth, index + 1)
            v += temp / len(state.getLegalActions(index))
        return v
